...
- Выход после cancel того же большого ответа завершает необходимую публикацию
  за 1,36–1,38 с при пределе 2 с и восстанавливает ICANON/ECHO/ISIG.
  Повторная проверка прежней реализации выявила превышение лимита (~2,09 с):
  завершение инвалидировало кеш Markdown, а worker заново строил всю физическую
  раскладку. Теперь source revision отделён от outcome, preview сохраняет origins,
  а неизменённые физические строки разделяются между снимками через Arc.
  После исправления три дополнительных последовательных запуска дали
  1,364 / 1,371 / 1,359 с. Лимит теста не менялся.
...
Все три команды завершились успешно. После исправления кеширования полный набор
с `--all-features --no-fail-fast`: **401 unit + 14 PTY/CLI +
1 stdout = 416 passed**, 0 failed, 2 ignored. Дополнительно прошёл
`cargo clippy --locked --offline --all-targets --all-features -- -D warnings`.
`--locked --offline` подтвердил
